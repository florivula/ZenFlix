<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Pre-Order Movies</title>
    <link rel="stylesheet" href="css/pre_order.css">
</head>
<body>
    <a href="home.php" class="logo">
        Zen<span>Flix</span>
    </a>
    <header>
        <h1>Pre-Order Movies</h1>
    </header>
    <main>
        <div class="row">
            <div class="movie">
                <img src="img/movie-1.jpg" alt="Movie 1">
                <h2>Movie 1</h2>
                <p>Release Date: 1/1/2023</p>
                <button>Pre-Order Now</button>
            </div>
            <div class="movie">
                <img src="img/movie-2.jpg" alt="Movie 2">
                <h2>Movie 2</h2>
                <p>Release Date: 2/1/2023</p>
                <button>Pre-Order Now</button>
            </div>
            <div class="movie">
                <img src="img/movie-3.jpg" alt="Movie 3">
                <h2>Movie 3</h2>
                <p>Release Date: 3/1/2023</p>
                <button>Pre-Order Now</button>
            </div>
        </div>
        <div class="row">
            <div class="movie">
                <img src="img/movie-4.jpg" alt="Movie 4">
                <h2>Movie 4</h2>
                <p>Release Date: 4/1/2023</p>
                <button>Pre-Order Now</button>
            </div>
            <div class="movie">
                <img src="img/movie-5.jpg" alt="Movie 5">
                <h2>Movie 5</h2>
                <p>Release Date: 5/1/2023</p>
                <button>Pre-Order Now</button>
            </div>
            <div class="movie">
                <img src="img/movie-6.jpg" alt="Movie 6">
                <h2>Movie 6</h2>
                <p>Release Date: 6/1/2023</p>
                <button>Pre-Order Now</button>
            </div>
        </div>
        <div class="row">
            <div class="movie">
                <img src="img/movie-7.jpg" alt="Movie 7">
                <h2>Movie 7</h2>
                <p>Release Date: 7/1/2023</p>
                <button>Pre-Order Now</button>
            </div>
            <div class="movie">
                <img src="img/movie-8.jpg" alt="Movie 8">
                <h2>Movie 8</h2>
                <p>Release Date: 8/1/2023</p>
                <button>Pre-Order Now</button>
            </div>
            <div class="movie">
                <img src="img/show-5.jpg" alt="Movie 9">
                <h2>Movie 9</h2>
                <p>Release Date: 9/1/2023</p>
                <button>Pre-Order Now</button>
            </div>
        </div>
    </main>
</body>
</html>